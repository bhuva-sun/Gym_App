...
async function check() {
  ...
  clansSnap.forEach(doc => {
    const data = doc.data();
    console.log(`- ${doc.id}: ${data.name} (Code: ${data.inviteCode})`);
  });

  const authUsersSnap = await getDocs(collection(db, "authUsers"));
  console.log("\nAuthUsers:");
  authUsersSnap.forEach(doc => {
    const data = doc.data();
    console.log(`- ${doc.id}: ${data.email} | clanId: ${data.clanId} | memberId: ${data.memberId}`);
  });
  ...
  membersSnap.forEach(doc => {
    const data = doc.data();
    console.log(`- ${doc.id}: ${data.email} | clanId: ${data.clanId}`);
  });
  process.exit(0);
}
